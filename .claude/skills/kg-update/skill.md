---
name: kg-update
description: "触发: 当用户说 '图谱''知识图谱''更新图谱''关系图''查关系' 时激活。扫描项目文件，批量同步到 Neo4j 知识图谱。用法: /kg-update [full|人物|世界观|伏笔]"
---
...
使用 MCP 工具按顺序写入：
1. **创建节点**：`mcp__neo4j-memory__create_entities` 批量创建
2. **添加观察信息**：`mcp__neo4j-memory__add_observations` 补充属性
3. **创建关系**：`mcp__neo4j-memory__create_relations` 批量连接

### 4. 验证与报告

- 用 `mcp__neo4j-memory__read_graph` 读取当前图谱统计
- 向用户报告同步结果：
  - 新增节点数（按类型）
  - 新增关系数（按类型）
  - 可能的冲突或遗漏
